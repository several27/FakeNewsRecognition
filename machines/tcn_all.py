...
from machines.data_generator import embedded_news_generator, path_data, path_news_train, path_news_val, \
    path_news_shuffled, path_fasttext_jsonl, embedded_news_generator_all, path_news_train_all, path_news_val_all, \
    news_labels
...
def train():
    ...
    with tf.device('/gpu:0'):
        model = tcn_model(num_channels=[1000] * 4 + [100])
        model.summary()
        ...
        print('Training...')
        model.fit_generator(embedded_news_generator_all(path_news_train_all, batch_size, fasttext_dict, max_words),
                            steps_per_epoch=train_size // batch_size, epochs=epochs, verbose=1,
                            validation_data=embedded_news_generator_all(path_news_val_all, batch_size, fasttext_dict,
                                                                        max_words),
