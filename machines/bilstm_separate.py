...
def bilstm_separate_model(units_title=(64,), units_content=(64,), units=(64,), dropout=(0.5,), hidden_dims=(18,)):
    ...
    bilstm_title = Bidirectional(CuDNNLSTM(units_title[0], stateful=True))(model_input_title)
    bilstm_content = Bidirectional(CuDNNLSTM(units_content[0], stateful=True))(model_input_content)

    merged = merge([bilstm_title, bilstm_content], mode='concat', concat_axis=-1)

    previous_layer = merged
    for h in hidden_dims:
        previous_layer = Dense(h, activation='relu')(previous_layer)

    model_output = Dense(1, activation='sigmoid')(previous_layer)
...
def train():
    with tf.device('/gpu:0'):
        model = bilstm_separate_model(hidden_dims=(128, 64, 32, 18))
        model.summary()
    ...
    with tf.device('/gpu:0'):
        ...
        file_weights = 'bilstm_separate_weights_deep_dense.{epoch:03d}-{val_acc:.4f}.hdf5'
        params = 'units_title=(64,), units_content=(64,), units=(64,), dropout=(0.5,), hidden_dims=(128, 64, 32, 18)'
