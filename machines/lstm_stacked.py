...
def lstm_model(units=(64,), dropout=(0.5,), hidden_dims=17):
    ...
    for i, u in enumerate(units):
        if i != (len(units) - 1):
            lstm = CuDNNLSTM(u, return_sequences=True, stateful=True)(previous_layer)
        else:
            lstm = CuDNNLSTM(u, stateful=True)(previous_layer)
...
def train():
    ...
    with tf.device('/gpu:0'):
        ...
        checkpoint = ModelCheckpoint(path_data + 'lstm_stacked_128_weights.{epoch:03d}-{val_acc:.4f}.hdf5',
